...
def cmd_init(lon,lat):
    ...
    if lat < 0:
        ...
        c_lat = "N" + str(s_lat)
    cmd="aws s3 cp --no-sign-request" + " s3://copernicus-dem-{0}m/Copernicus_DSM_COG_{1}_{2}_00_{3}_00_DEM/ {4} --recursive".format(str(resolutions),str(int(resolutions/3)),str(c_lat),str(c_lon),savePath)
    return cmd
